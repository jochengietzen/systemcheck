...
public class LoadHistory {
    ...
    public static void recordCurrentEntry(int countOfKeepings) {
        ...
        Computer computer = Hudson.getInstance().getComputer("(master)");
        LOGGER.log(Level.FINE, "Starting Masterload History");

        String historyFileString = createHistoryFiles(countOfKeepings);
        LOGGER.log(Level.FINE, "Get new Masterload...");
        String load = "0";
        boolean loadCheckOkay = true;
        try {
            load = computer.getChannel().call(new MonitorTask());
        } catch (Exception e) {
            loadCheckOkay = false;
            e.printStackTrace();
        }
        boolean work = true;
        try {
            LOGGER.log(Level.FINE, "Starting to write Information now!");
            LOGGER.log(Level.FINE, "Start writing history...");
            if (loadCheckOkay) {
                writeData(load, countOfKeepings, historyFileString);
            } else {
                writeData(null, countOfKeepings, historyFileString);
            }
        } catch (IOException e) {
            LOGGER.warning("Writing history file failed!");
            work = false;
        }
...
//        }
    ...
    private static void saveHistoryFile(String historyFileString) throws IOException {
        BufferedWriter bw = new BufferedWriter(new FileWriter(new File(Constants.history_file_Load)));
